...
- **长对话卡死** — 历史会话改为批量写入状态，避免逐条消息触发整页重复渲染。
- **大历史记录渲染** — 默认只渲染最近 200 条消息，并支持每次加载更早的 200 条，降低大型会话的 DOM 和内存压力。
- **窗口无法关闭** — 移除依赖 WebView 响应的关闭确认；退出时由原生事件清理输入句柄并非阻塞终止托管 CLI 子进程。

### Improved

- 工具调用分组增加组件记忆化，减少长会话中的无效重复渲染。

### Validation

- 10 个前端单元测试通过。
- TypeScript、Vite、Rust 检查通过；Windows 应用与安装包构建完成（本机未配置更新签名私钥）。
...
- **LaTeX 数学公式渲染** — 聊天消息和 Markdown 预览支持 KaTeX 行内/块级公式，并兼容 Claude 常用的 `\(...\)` 与 `\[...\]` 分隔符。
- **功能面板横向滑条** — 右侧面板较窄时可横向浏览文件、预览、技能和插件标签，关闭按钮保持固定。

### Fixed

- **1M 上下文仪表计算** — 使用最近一次模型调用的输入、缓存读取、缓存创建和输出 Token 计算真实占用，避免工具调用累计导致乱跳或错误归零。
- **历史会话上下文恢复** — 加载已有对话时恢复最后一次有效用量与模型，不再错误显示 100% 可用。
- **重复“最新”按钮** — 移除输入框上方的重复入口，仅保留右侧时间轴按钮。

### Validation

- 6 个前端单元测试通过。
- TypeScript、Vite 与 Windows Tauri Release 构建通过。
